import { View, Text, ImageBackground, useWindowDimensions, TouchableOpacity } from "react-native";
      ...
      >
        ...
        <View style={{ paddingTop: insets.top + 24 }} className="px-4">
          <View className="flex-row flex-wrap gap-2">
            ...
            <View className="px-3 py-1.5 rounded-full bg-black/30 backdrop-blur-md border border-white/30">
              <Text className="text-white text-xs font-medium">{recipe.difficulty}</Text>
            </View>
              {/* sliced them up so we have individual boxes */}
              {recipe.tags.slice(0, 2).map((tag, index) => (
                <View key={index} className="px-3 py-1.5 rounded-full bg-black/30 backdrop-blur-md border border-white/30">
        ...
        >
          ...
          <View className="items-end mb-4 pr-2">
            <View className="items-center">
              <TouchableOpacity className="bg-white rounded-full p-3 mb-2 ">
                <ChevronRight size={24} color="#000" />
              </TouchableOpacity>
            <Text className="text-white text-xs font-medium">See full recipe</Text>
            </View>
          </View>
